...
**Открытый вопрос 5 (новый).** Почему два `<style>` приезжают пустыми, если CSSOM jsdom принимает
все нужные операции. Обе пустые таблицы наполняются через `Style` из compose-html, обе непустые —
...
**Риск 9 — контекст запроса до рендера не доезжает (§1.11).** Строка запроса, cookies, язык и
авторизация теряются, ответ приходит с кодом 200 и попадает в кэш, так что неверная страница
раздаётся дальше всем. Механизм смягчения на первое время — **отказаться обслуживать** то, что
заведомо будет отрендерено неверно: если у запроса есть строка запроса или cookies, а маршрут не
объявлен как от них не зависящий, отступать к Kobweb, а не молча рендерить. Дороже, но честно;
делать до того, как SSR увидит вторую страницу.
