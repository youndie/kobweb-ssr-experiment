...
**Риск 9 — контекст запроса до рендера не доезжает (§1.11).** Смягчён в M5-01 (§1.12): плагин
отказывается обслуживать запрос, несущий то, что рендерер выбросит, и говорит в логе, из-за чего.
Остаточный риск: отказ — это отсутствие SSR, а не SSR. Пока не сделана M5-02, любая страница,
читающая строку запроса, остаётся клиентской, и польза ограничена страницами, которые от запроса
не зависят.
