...
| ~~Память рендерера линейна по числу воркеров: **≈ 115 МиБ постоянных + ≈ 342 МиБ на воркер**~~ — **завышено в 2.4 раза, см. правку ниже**. Линейность подтвердилась, величина нет | `ps` по дереву процессов рендерера |
...
**Следствие 3. Размер пула упирается в память, а не в ядра** — вывод верный, величина была неверна.
Восемь ядер уже на четырёх воркерах дают недолинейный прирост по времени: одиночный рендер дорожает
с 700 до 820 мс. Обе границы надо держать в виду, и обе — про эту машину.

**Правка при подготовке деплоя (2026-08-24): «342 МиБ на воркер» завышено в 2.4 раза.** Замер
складывал RSS по дереву процессов рендерера, а RSS считает страницы, разделяемые между процессами
Chromium, **по разу на каждый процесс**. Учёт по cgroup, где страница считается один раз, даёт
другое:

| воркеров | простой | под нагрузкой |
|---|---|---|
| 1 | ≈180 МиБ | 212 МиБ |
| 2 | 313 | 357 |
| 4 | 571 | 643 |

Это **≈68 МиБ постоянных + ≈144 МиБ на воркер**, и подгонка ложится точно: 68 + 144·1 = 212,
68 + 144·2 = 356, 68 + 144·4 = 644. Контейнер сайта — около 190 МиБ под нагрузкой.

Ошибка стоила не только строчки в документе: из неё были выставлены `resources` в чарте, и кластер
резервировал 960 МиБ запросов при потолке в 2 ГиБ на то, что укладывается в 400. Запросы держат
ёмкость узла независимо от того, используется она или нет, — то есть неверное число вычиталось из
кластера постоянно. Проверено на ужатых лимитах: три круга по восемь параллельных рендеров плюс все
страницы, обе половины устоялись ниже 195 МиБ, OOMKilled нет.

**Урок тот же, что в §1.9, и это второй раз за проект: сумма RSS по дереву процессов — не мера
потребления.** Для процессов, разделяющих память, она завышает тем сильнее, чем их больше — то есть
ровно в ту сторону, которая делает вывод «дорого» убедительнее.
